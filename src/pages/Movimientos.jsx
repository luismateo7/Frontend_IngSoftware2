...
function Movimiento() {
  ...
  const eliminarGasto = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:4000/api/movimientos/eliminar-movimiento/${id}`, {}, config);
  ...
  useEffect(() => {
    if (!localStorage.getItem('auth')) {
      ...
      const getMovimientos = async () => {
        try {
          const { data } = await axios.get('http://localhost:4000/api/movimientos/listar-movimientos', config);
          setGastos(data);
        } catch (error) {
